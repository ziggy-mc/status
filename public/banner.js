 ...
 *
 * On load it fetches /api/banner. If there are active incidents it injects a
 * fixed banner at the top of the page with a close (✕) button that persists
 * the dismissed state to localStorage.
 */
(function () {
  "use strict";

  var BANNER_DISMISSED_KEY = "incidentBannerClosed";
  ...
  function createBanner(data) {
    ...
    // Push page content down after the browser has applied styles and completed layout
  ...
  function init() {
    ...
    fetch("/api/banner")
      .then(function (res) {
        if (!res.ok) {
          console.error("Banner API responded with " + res.status + " " + res.statusText);
          return null;
        }
        return res.json();
      })
